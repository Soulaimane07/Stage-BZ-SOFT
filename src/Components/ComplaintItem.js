function ComplaintItem({type, Language, data, id, setComplaintBody}) {  
  return (
    <div key={id} className="mx-auto max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="p-5">
            ...
            <div className='mt-6 bg-orange-300 w-full'>
              <button onClick={()=> setComplaintBody(data)} className={` mb-6 ${type === "ar" ? "float-right flex-row-reverse" : "float-left"} inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-orange-700 rounded-lg hover:bg-orange-800 focus:ring-4 focus:outline-none focus:ring-orange-300 dark:bg-orange-600 dark:hover:bg-orange-700 dark:focus:ring-orange-800`}>
                  {Language?.read}
                  {type === "ar"
